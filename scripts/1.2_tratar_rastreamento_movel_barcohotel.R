...
diretorio = 'dados/rastreamento_movel_barcohotel'
arquivos <- list.files(diretorio)
marcacao <- carregar_dados(schema = 'telemetria', table = 'marcacao')
df_bi <- read.csv('dados/filtro/transmissor.csv', sep = ";") %>% dplyr::select(-transmissor_id,transmissor_id = radio_id, bi = intervalo_radio)
...
for (arquivo in arquivos) {
  ...
  dados_iii = read.table(file.path(diretorio, arquivo), col.names = c('data_or', 'hora', 'canal','id',  'antena', 'potencia', 'lat', 'long'))
  ...
  dados_iii =
    dados_iii %>%
    dplyr::mutate(
      data_diff = data - min(dados_iii$data),
  ...
  movel_bruto = rbind(movel_bruto, dados_iii)
}

# Filtra os dados brutos
movel_filtrado_barcohotel <-
  movel_bruto %>%
...
write.csv(movel_filtrado_barcohotel, 'dados/movel_filtrado/movel_barcohotel.csv', row.names = FALSE)

## Inserir os dados no banco de dados
inserir_dados(movel_filtrado_barcohotel, schema = 'telemetria', table = 'deteccao_radio_movel_barcohotel')
