...
dados %>%
  ...
  dplyr::summarise(
    ...
    taxa_desloc_media_kmrd = sum(abs(deslocamento), na.rm = TRUE)/sum(tempo[tempo > 0], na.rm = TRUE),
    taxa_desloc_mont_media_kmrd = sum(abs(deslocamento[sentido == "Montante"]), na.rm = TRUE)/sum(tempo[sentido == "Montante" & tempo > 0], na.rm = TRUE),
    ...
    taxa_desloc_jus_media_kmrd = sum(abs(deslocamento[sentido == "Jusante"]), na.rm = TRUE)/sum(tempo[sentido == "Jusante" & tempo > 0], na.rm = TRUE),
